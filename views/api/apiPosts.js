...
export const getAll = async (index, dispatch) => {
  try {
    ...
    const res = await axios({
      url: index.messageId
        ? `/api/getAllPaging?page=${index.page}&size=${index.size}&messageId=${index.messageId}&channel=${index.channel}`
        : `/api/getAllPaging?page=${index.page}&size=${index.size}&channel=${index.channel}`,
      method: 'GET',
    });
...
export const getOne = async (index, dispatch) => {
  try {
    ...
    const res = await axios({
      url: index.id
        ? `/api/posts?messageId=${index.messageId}&id=${index.id}`
        : `/api/posts?messageId=${index.messageId}`,
      method: 'GET',
    });
    dispatch({ type: 'SET_POSTS', payload: res.data });
  } catch (error) {

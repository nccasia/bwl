    ...
    const handleScroll = () => {
      if (
        ...
        if (
          !state.loadingUsers &&
          state.pageUsers !== -1 &&
          state.lengthUsers > 10
        ) {
          useDataDebouncer(
            dispatch({
              type: 'CHANGE_PAGE_USERS',
              payload: { page: state.pageUsers + 1, length: state.lengthUsers },
            }),
            500,
          );
        }
    ...
    >
      ...
      <div className="content-right-header">
        {state.search === '' &&
          ...
          state.searchUsersPosts === '' && <h1>Channel</h1>}
        {state.search !== '' &&
          ...
          state.searchTime?.length !== 2 && (
            <h1>
              ...
              {state.loadingUsers ? (
                <CircularProgress
                  sx={{ color: 'rgb(108, 117, 136)', fontSize: '12px' }}
                  size={15}
                />
              ) : (
                state.lengthUsers
      ...
      >
        {state.search === '' &&
          state.searchTime?.length !== 2 &&
          state.searchUsersPosts === '' && <OnlineUsers type="channelList" />}
        {state.search !== '' &&
          state.searchUsersPosts === '' &&
          state.searchTime?.length !== 2 && <OnlineUsers type="searchUsers" />}
        {state.searchUsersPosts !== '' &&
          state.search !== '' &&
          state.searchTime?.length !== 2 && (
            <SearchPost type="searchUsersPosts" />
          )}
        {state.searchTime?.length === 2 && state.search === '' && (
          <SearchPost type="searchTimePosts" />
        )}
        {state.loadingUsers && (
